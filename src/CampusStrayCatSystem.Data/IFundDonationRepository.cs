...
namespace CampusStrayCatSystem.Data
{
    ...
    public interface IFundDonationRepository
    {
        ...
        Task<IEnumerable<FundDonation>> GetByProject(string projectId); // 按项目 ID 查询捐赠记录
        Task<IEnumerable<FundDonation>> GetForDisclosureByProject(string projectId); // 查询财务公示捐赠记录，匿名捐赠人脱敏
        Task<IEnumerable<FundDonation>> GetByDonor(string donorUserId); // 按捐赠人查询其捐赠记录
        Task<bool> CreateWithRaisedUpdate(FundDonation donation);       // 记录捐赠并累加已筹金额；项目非 ACTIVE 时返回 false
