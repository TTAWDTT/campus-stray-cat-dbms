...
type RescueModal = 'tnr' | 'medical' | 'emergency' | 'missing' | 'status' | 'logs' | 'tnr-detail' | 'medical-detail' | 'reminder-detail' | 'emergency-detail' | 'missing-detail' | null;
...
function TnrCatSelect({ options, value, onChange }: { options: Array<{ key: string; label: string }>; value: string; onChange: (value: string) => void }) {
  ...
  const syncScrollbar = () => {
    ...
    const height = visible ? Math.max(28, Math.round(element.clientHeight * element.clientHeight / element.scrollHeight)) : 0;
    const distance = element.scrollHeight - element.clientHeight;
    const top = distance ? Math.round(element.scrollTop / distance * (element.clientHeight - height)) : 0;
  ...
  return <div className="rescue-cat-picker" ref={picker}>
    <button type="button" className={`rescue-cat-trigger ${open ? 'open' : ''}`} aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)}>
      <span>{selected?.label || '请选择需要救助的猫咪'}</span><i aria-hidden="true" />
    </button>
    {open && <div className="rescue-cat-menu" role="listbox" aria-label="选择需要救助的猫咪">
      <div className="rescue-cat-options" ref={list} onScroll={syncScrollbar}>
        {options.map((option) => <button type="button" role="option" aria-selected={option.key === value} className={option.key === value ? 'active' : ''} key={option.key} onClick={() => { onChange(option.key); setOpen(false); }}>{option.label}</button>)}
      </div>
      {scrollbar.visible && <span className="rescue-cat-scroll-track" aria-hidden="true"><i style={{ height: scrollbar.height, transform: `translateY(${scrollbar.top}px)` }} /></span>}
    </div>}
  </div>;
}
...
export function RescuePage() {
  ...
  const filteredTnrCases = tnrCases.filter((item) => `${catName(item.catID)} ${item.hospitalName || ''} ${tnrLabels[item.currentStatus || ''] || item.currentStatus || ''}`.toLowerCase().includes(detailSearch.toLowerCase()));
  const filteredMedicalRecords = medicalRecords.filter((item) => `${catName(item.catID)} ${item.hospitalName || ''} ${item.diagnosis || ''} ${medicalLabels[item.recordType || ''] || item.recordType || ''}`.toLowerCase().includes(detailSearch.toLowerCase()));
  const filteredReminders = reminders.filter((item) => `${catName(item.catID)} ${reminderLabels[item.reminderType] || item.reminderType} ${item.sendStatus}`.toLowerCase().includes(detailSearch.toLowerCase()));
  const filteredEmergencies = emergencies.filter((item) => `${areaName(item.areaID)} ${urgencyLabels[item.urgencyLevel]} ${emergencyLabels[item.processStatus] || item.processStatus} ${item.processResult || ''}`.toLowerCase().includes(detailSearch.toLowerCase()));
  const filteredAlerts = alerts.filter((item) => `${catName(item.catID)} ${missingLabels[item.alertStatus] || item.alertStatus} ${item.remark || ''}`.toLowerCase().includes(detailSearch.toLowerCase()));
  const openDetail = (kind: RescueModal) => { setDetailSearch(''); openModal(kind); };
  const moduleCard = (title: string, note: string, icon: 'icon-camera' | 'icon-chat' | 'icon-critterpedia' | 'icon-diy' | 'icon-miles' | 'icon-shopping', latest: string, count: number, onClick: () => void, tone: 'teal' | 'yellow' | 'red' = 'teal') => (
    <Card hoverable className={`rescue-module-card ${tone}`} style={{ display: 'grid', gridTemplateColumns: '42px minmax(0,1fr) auto', gap: 12, alignItems: 'center', minHeight: 112, padding: 16 }} onClick={onClick} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onClick(); }}>
      <div className="rescue-module-card-icon" style={{ display: 'grid', width: 42, height: 42, placeItems: 'center', borderRadius: 14, color: tone === 'red' ? 'var(--animal-error-color)' : 'var(--animal-primary-color)', background: tone === 'red' ? 'rgba(255,234,229,.86)' : 'var(--animal-primary-color-bg)' }}><Icon name={icon} size={21} /></div>
      <div className="rescue-module-card-copy" style={{ display: 'grid', gap: 4, minWidth: 0 }}><strong style={{ color: 'var(--animal-text-color)', fontSize: 14 }}>{title}</strong><small style={{ color: 'var(--animal-text-color-secondary)', fontSize: 11 }}>{note}</small><span style={{ overflow: 'hidden', color: 'var(--animal-text-color)', fontSize: 12, textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{latest}</span></div>
      <div className="rescue-module-card-count" style={{ display: 'grid', justifyItems: 'end', gap: 2 }}><b style={{ color: 'var(--animal-text-color)', fontSize: 24, lineHeight: 1 }}>{count}</b><em style={{ color: 'var(--animal-text-color-secondary)', fontSize: 10, fontStyle: 'normal' }}>条记录</em></div>
    </Card>
  );
  ...
  return (
    <section className="feature-page rescue-page">
      ...
      <Card className="rescue-workspace-card"><div className="rescue-tabs">{careTabs.map((item) => <button key={item.key} className={tab === item.key ? 'active' : ''} onClick={() => setTab(item.key as RescueTab)}><Icon name={item.icon} size={18} /><span><strong>{item.label}</strong><small>{item.note}</small></span></button>)}</div>
        {tab === 'care' && <div className="rescue-panel"><div className="rescue-panel-heading"><div><h2>救助与护理</h2></div><Button type="default" icon={<Icon name="icon-camera" size={15} />} onClick={resetTnr}>新建案例</Button></div><div className="rescue-module-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 12 }}>
          {moduleCard('TNR 救助流程', '捕捉、绝育与放归状态', 'icon-diy', tnrCases[0] ? `${catName(tnrCases[0].catID)} · ${tnrLabels[tnrCases[0].currentStatus || ''] || '待处理'}` : '暂时没有救助案例', tnrCases.length, () => openDetail('tnr-detail'))}
          {moduleCard('医疗历史', '猫咪就诊与健康记录', 'icon-camera', medicalRecords[0] ? `${catName(medicalRecords[0].catID)} · ${medicalLabels[medicalRecords[0].recordType || ''] || '医疗记录'}` : '暂时没有医疗记录', medicalRecords.length, () => openDetail('medical-detail'))}
          {moduleCard('护理提醒', '疫苗、驱虫与复诊待办', 'icon-chat', pendingReminders[0] ? `${catName(pendingReminders[0].catID)} · ${reminderLabels[pendingReminders[0].reminderType] || pendingReminders[0].reminderType}` : '当前没有待处理提醒', pendingReminders.length, () => openDetail('reminder-detail'), 'yellow')}
        </div></div>}
        {tab === 'emergency' && <div className="rescue-panel"><div className="rescue-panel-heading"><div><h2>紧急救助</h2></div><Button type="primary" icon={<Icon name="icon-miles" size={15} />} onClick={resetEmergency}>紧急上报</Button></div><div className="rescue-module-grid single" style={{ display: 'grid', gap: 12 }}>{moduleCard('紧急救助上报', '受伤、被困与高风险情况', 'icon-miles', emergencies[0] ? `${areaName(emergencies[0].areaID)} · ${urgencyLabels[emergencies[0].urgencyLevel] || emergencies[0].urgencyLevel}级` : '当前没有上报记录', emergencies.length, () => openDetail('emergency-detail'), 'red')}</div></div>}
        {tab === 'missing' && <div className="rescue-panel"><div className="rescue-panel-heading"><div><h2>猫咪失踪预警</h2></div><Button type="primary" icon={<Icon name="icon-map" size={15} />} onClick={resetMissing}>发布预警</Button></div><div className="rescue-module-grid single" style={{ display: 'grid', gap: 12 }}>{moduleCard('失踪预警', '记录最后目击并跟进寻回', 'icon-critterpedia', alerts[0] ? `${catName(alerts[0].catID)} · ${missingLabels[alerts[0].alertStatus] || alerts[0].alertStatus}` : '当前没有失踪预警', alerts.length, () => openDetail('missing-detail'), 'yellow')}</div></div>}
      </Card>
      <Modal open={modal === 'tnr'} title="新建 TNR 救助案例" width={650} typewriter={false} onClose={closeModal} footer={<div className="rescue-modal-footer"><Button type="default" onClick={closeModal} disabled={saving}>取消</Button><Button type="primary" onClick={() => void saveTnr()} loading={saving}>加入救助流程</Button></div>}><div className="rescue-form"><div className="rescue-form-intro"><Icon name="icon-camera" size={22} /><span><strong>从发现开始记录这条救助线</strong><small>案例创建后可在表格中持续推进状态并查看完整日志。</small></span></div><div className="rescue-form-grid"><label className="rescue-cat-select"><span>猫咪 *</span><TnrCatSelect options={catOptions} value={tnrForm.catID} onChange={(value) => setTnrForm((form) => ({ ...form, catID: value }))} /></label><label><span>就诊医院</span><Input value={tnrForm.hospitalName} onChange={(event) => setTnrForm((form) => ({ ...form, hospitalName: event.target.value }))} placeholder="例如：校动物医院" /></label><label><span>计划捕捉时间</span><Input type="datetime-local" value={tnrForm.captureTime} onChange={(event) => setTnrForm((form) => ({ ...form, captureTime: event.target.value }))} /></label><label><span>计划手术时间</span><Input type="datetime-local" value={tnrForm.surgeryTime} onChange={(event) => setTnrForm((form) => ({ ...form, surgeryTime: event.target.value }))} /></label><label><span>计划放归时间</span><Input type="datetime-local" value={tnrForm.releaseTime} onChange={(event) => setTnrForm((form) => ({ ...form, releaseTime: event.target.value }))} /></label><label><span>预计费用</span><Input type="number" value={tnrForm.totalCost} onChange={(event) => setTnrForm((form) => ({ ...form, totalCost: event.target.value }))} placeholder="元" /></label></div></div></Modal>
      ...
      <Modal open={modal === 'tnr-detail'} title="TNR 救助流程" width={900} typewriter={false} onClose={closeModal} footer={<div className="rescue-modal-footer"><Button type="primary" onClick={closeModal}>关闭</Button></div>}>
        <div className="rescue-detail-toolbar" style={{ marginBottom: 12 }}><Input value={detailSearch} onChange={(event) => setDetailSearch(event.target.value)} placeholder="筛选猫咪、医院或状态" /></div>
        <Table className="rescue-table-wide" columns={tnrColumns} dataSource={filteredTnrCases as unknown as Record<string, unknown>[]} rowKey="caseID" loading={loading} emptyText="还没有 TNR 救助案例" scroll={{ x: 760, y: 360 }} />
      </Modal>
      <Modal open={modal === 'medical-detail'} title="医疗历史" width={820} typewriter={false} onClose={closeModal} footer={<div className="rescue-modal-footer"><Button type="default" onClick={resetMedical}>新增记录</Button><Button type="primary" onClick={closeModal}>关闭</Button></div>}>
        <div className="rescue-detail-toolbar" style={{ marginBottom: 12 }}><Input value={detailSearch} onChange={(event) => setDetailSearch(event.target.value)} placeholder="筛选猫咪、诊断或医疗类型" /></div>
        <Table columns={medicalColumns} dataSource={filteredMedicalRecords as unknown as Record<string, unknown>[]} rowKey="recordID" loading={loading} emptyText="暂无医疗记录" scroll={{ x: 620, y: 360 }} />
      </Modal>
      <Modal open={modal === 'reminder-detail'} title="护理提醒" width={680} typewriter={false} onClose={closeModal} footer={<div className="rescue-modal-footer"><Button type="primary" onClick={closeModal}>关闭</Button></div>}>
        <div className="rescue-detail-toolbar" style={{ marginBottom: 12 }}><Input value={detailSearch} onChange={(event) => setDetailSearch(event.target.value)} placeholder="筛选猫咪、提醒类型或状态" /></div>
        {filteredReminders.length ? <div className="rescue-reminder-list">{filteredReminders.map((reminder) => <div key={reminder.reminderID}><span><strong>{catName(reminder.catID)} · {reminderLabels[reminder.reminderType] || reminder.reminderType}</strong><small>{formatTime(reminder.reminderTime, '暂未安排时间')} · {reminder.sendStatus === 'PENDING' ? '待发送' : reminder.sendStatus === 'SENT' ? '已发送' : '已完成'}</small></span><div>{reminder.sendStatus === 'PENDING' && <Button type="text" size="small" onClick={() => void completeReminder(reminder, 'sent')}>已发送</Button>}{reminder.sendStatus !== 'COMPLETED' && <Button type="text" size="small" onClick={() => void completeReminder(reminder, 'complete')}>完成</Button>}</div></div>)}</div> : <EmptyState icon="icon-camera" title="没有匹配的护理提醒" description="调整筛选条件后再试。" />}
      </Modal>
      <Modal open={modal === 'emergency-detail'} title="紧急救助上报" width={930} typewriter={false} onClose={closeModal} footer={<div className="rescue-modal-footer"><Button type="primary" onClick={closeModal}>关闭</Button></div>}>
        <div className="rescue-detail-toolbar" style={{ marginBottom: 12 }}><Input value={detailSearch} onChange={(event) => setDetailSearch(event.target.value)} placeholder="筛选地点、等级、进度或处理结果" /></div>
        <Table columns={emergencyColumns} dataSource={filteredEmergencies as unknown as Record<string, unknown>[]} rowKey="reportID" loading={loading} emptyText="暂时没有紧急救助上报" scroll={{ x: 760, y: 360 }} />
      </Modal>
      <Modal open={modal === 'missing-detail'} title="猫咪失踪预警" width={860} typewriter={false} onClose={closeModal} footer={<div className="rescue-modal-footer"><Button type="primary" onClick={closeModal}>关闭</Button></div>}>
        <div className="rescue-detail-toolbar" style={{ marginBottom: 12 }}><Input value={detailSearch} onChange={(event) => setDetailSearch(event.target.value)} placeholder="筛选猫咪、状态或说明" /></div>
        <Table columns={missingColumns} dataSource={filteredAlerts as unknown as Record<string, unknown>[]} rowKey="alertID" loading={loading} emptyText="当前没有失踪预警" scroll={{ x: 720, y: 360 }} />
      </Modal>
    </section>
  );
}
