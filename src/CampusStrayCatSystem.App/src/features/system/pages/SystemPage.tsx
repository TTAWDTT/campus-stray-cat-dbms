...
export function SystemPage() {
  ...
  const submitRole = async () => {
    if (!roleForm.roleID.trim() || !roleForm.description.trim()) {
      setError('请填写角色 ID 和角色说明。');
      return;
    }
    setSaving(true);
    try {
      const payload = { ...roleForm, roleID: roleForm.roleID.trim(), permissionScope: roleForm.permissionScope.trim() };
      if (editingRole) {
        await systemService.updateRole(editingRole.roleID, payload);
  ...
  return <section className="feature-page system-page">
    ...
    <Card className="system-workspace-card">
      ...
      {activeTab === 'users' && <div className="system-panel"><div className="system-panel-heading"><div><h2>用户管理</h2></div><Button type="primary" onClick={openNewUser}><Icon name="icon-diy" size={16} />新增用户</Button></div><div className="system-filter-grid"><Input value={userSearch} onChange={(event) => setUserSearch(event.target.value)} allowClear placeholder="搜索账号、姓名或学号" prefix={<Icon name="icon-chat" size={15} />} /><Select value={userRole} onChange={setUserRole} options={[{ key: '', label: '全部角色' }, ...roles.map((role) => ({ key: role.roleID, label: roleNames[role.roleName] || role.roleName }))]} /><Select value={userStatus} onChange={setUserStatus} options={[{ key: '', label: '全部账号状态' }, { key: 'ACTIVE', label: '正常使用' }, { key: 'DISABLED', label: '已停用' }]} /><Button type="default" onClick={() => { setUserSearch(''); setUserRole(''); setUserStatus(''); }}>重置</Button></div>{!loading && !filteredUsers.length ? <EmptyState icon="icon-chat" title="没有符合条件的用户" description="调整筛选条件，或直接创建一个新的系统账号。" action={<Button type="primary" onClick={openNewUser}>新增用户</Button>} /> : <Table columns={userColumns} dataSource={filteredUsers as unknown as Record<string, unknown>[]} rowKey="userID" loading={loading} emptyText="暂无用户数据" scroll={{ x: 780 }} />}</div>}
      {activeTab === 'roles' && <div className="system-panel"><div className="system-panel-heading"><div><h2>角色与权限</h2><p>角色编码使用系统约定的英文值，中文仅用于页面显示与说明。</p></div><Button type="primary" onClick={openNewRole} disabled={!availableRoleNames.length}><Icon name="icon-diy" size={16} />新增角色</Button></div>{!loading && !roles.length ? <EmptyState icon="icon-design" title="还没有可用角色" description="先创建角色，再为系统用户分配相应权限。" action={<Button type="primary" onClick={openNewRole}>新增角色</Button>} /> : <Table columns={roleColumns} dataSource={roles as unknown as Record<string, unknown>[]} rowKey="roleID" loading={loading} emptyText="暂无角色数据" scroll={{ x: 680 }} />}</div>}
      {activeTab === 'blacklist' && <div className="system-panel"><div className="system-panel-heading"><div><h2>领养黑名单</h2><p>记录不适合领养或存在违规行为的用户，供审核流程核验。</p></div><Button type="primary" onClick={openBlacklist}><Icon name="icon-diy" size={16} />加入黑名单</Button></div><div className="system-filter-grid system-blacklist-filters"><Input value={blacklistSearch} onChange={(event) => setBlacklistSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void reloadBlacklist(); }} allowClear placeholder="搜索用户、原因或关联申请" prefix={<Icon name="icon-critterpedia" size={15} />} /><Select value={blacklistStatus} onChange={setBlacklistStatus} options={[{ key: '', label: '全部状态' }, { key: 'ACTIVE', label: '生效中' }, { key: 'RELEASED', label: '已解除' }]} /><Button type="default" onClick={() => void reloadBlacklist()}>查询</Button></div><div className="system-table-meta">当前共 {blacklistTotal} 条记录</div>{!loading && !blacklist.length ? <EmptyState icon="icon-critterpedia" title="没有符合条件的黑名单记录" description="需要时可将存在风险的用户加入黑名单。" action={<Button type="primary" onClick={openBlacklist}>加入黑名单</Button>} /> : <Table columns={blacklistColumns} dataSource={blacklist as unknown as Record<string, unknown>[]} rowKey="blacklistId" loading={loading} emptyText="暂无黑名单数据" scroll={{ x: 840 }} />}</div>}
    </Card>
